...
public abstract class ScriptTest
{
    ...
    protected static IReadOnlyList<Diagnostic> Errors<TGlobals, TResult>(string source)
    {
        using var engine = new ScriptEngine(Options);

        var diagnostics = engine.TryCompile<TGlobals, TResult>(source).Diagnostics;
    ...
    protected static void AssertError<TGlobals, TResult>(string source, ErrorCode expected)
    {
        var diagnostics = Errors<TGlobals, TResult>(source);
        Assert.True(
