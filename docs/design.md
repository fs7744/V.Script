...
重载与用户定义转换、对象创建（含 `new Func<...>(lambda)` 这类委托创建）、对象与集合初始化器、
数组创建三种写法（`new T[n]`、`new T[] { ... }`、`new[] { ... }`）、**集合表达式** `[a, b, c]`、
插值字符串（含对齐与格式说明符）、显式类型实参、**`nameof`**、**`default` 与 `default(T)`**、
throw 表达式、`if`/`while`/`do`/`for`/`foreach`/`break`/`continue`/`return`、`switch` 语句、
`try`/`catch`/`finally`/`throw`、`await`、lambda 与闭包（表达式体与块体，形参可写类型）、
**局部函数**（含递归与互递归）、泛型方法推断、扩展方法、模式匹配与 `switch` 表达式、锯齿数组。
...
| 集合表达式中的展开 | `[..xs, 1]` | 元素只能是单个表达式 |
| 嵌套初始化器 | `new A { Inner = { X = 1 } }` | 成员初始化器右侧必须是表达式 |
| 索引初始化器 | `new D { ["k"] = 1 }` | 未支持 |
| 逐字 / 原始字符串 | `@"a\b"` / `"""abc"""` | 词法未支持 |
| `goto` / `goto case` / `lock` / `using` 语句 | — | 未支持 |
...
- **`switch` 分支不能落空**：C# 里落空是编译错误，引擎同样报错（VS3006），但引擎的检查更保守——
  只有 `return`/`throw`/`break`/`continue` 以及必定跳转的 `if`/`try` 才算合法结尾
...
- **集合表达式转 `List<T>` 走 `Add`**，C# 走 span 拷贝——结果一样，见 §12 的实测差距
- **局部函数是委托，不是方法**：签名必须能表示成 `Func`/`Action`（≤16 个参数、无 `ref`/`out`、
  无泛型形参、无默认值与 `params`）。好处是它可以直接当委托传给 LINQ，抵消了方法组推断的缺失
...
- **方法组推断**：`xs.Select(Foo)` 不支持，必须写成 lambda；不过局部函数本身就是委托变量，
  `int Foo(int x) => ...; xs.Select(Foo)` 是可以的
...
1. **元组与解构**——多返回值的常见写法，也是位置模式 `is (a, b)` 的前置
2. **`using` 语句**与 **`out` 参数**——与外部 API 打交道时最容易碰壁的两处
3. **方法组推断** `xs.Select(Foo)`——局部函数已经能直接传，剩下的是普通方法
4. **逐字与原始字符串**——`@"..."` 在写正则或路径的脚本里很常用
