...
一个作用域捕获了什么，要到绑定结束才知道；而同步载体是一个 `DynamicMethod`，没有模块可以发射
布局类型。所以布局预先声明成泛型，按槽位数各一个：

```csharp
sealed class ScriptClosure<T0>          : ScriptClosure { public T0 Slot0; }
sealed class ScriptClosure<T0, T1>      : ScriptClosure { public T0 Slot0; public T1 Slot1; }
// ……到 4 个槽为止，再多退回 ArrayClosure，所有槽装箱存进 object[]
```

槽位是**公开字段**而不是属性：生成代码用 `ldfld` 读、`stfld` 写，也能用 `ldflda` 取地址。
引用类型的槽共用同一份泛型实例化，只有值类型才会让运行时多 JIT 一份，所以 4 这个上限是在
覆盖面和实例化数量之间取的——脚本实际捕获的通常是 globals 加一两个局部。

创建闭包时那个 IL 局部**用具体类型声明**，所以外层方法读写自己作用域的槽不需要任何转换。
只有 lambda 需要一次 `castclass`：它拿到的 `ldarg_0` 是基类型。对 sealed 类的 `castclass`
是一次类型句柄比较，比它替掉的 `callvirt get_Values` + `ldelem.ref`（带边界检查）+
`unbox.any` 便宜。数字在 §12。

### 捕获型 lambda 每次求值绑定一次委托

lambda 的方法体是一个独立的方法，第一个参数是闭包。生成代码没法对它 `ldftn`，所以委托由宿主
构造。不捕获的 lambda 在编译期就绑好一个共享委托；捕获型每次求值都要绑一个新的，因为闭包是
每次求值新建的。

绑定方式有两条路，`ClosureBinder` 走的是第一条：

| | 绑定 | 绑定 + 调用 8 次 | 只调用 8 次 | 绑定分配 |
|---|---:|---:|---:|---:|
| 编译期建开放委托，求值时包一层 | **14.7 ns** | **48.1 ns** | **27.6 ns** | 88 B |
| 求值时 `CreateDelegate(type, closure)` | 399.5 ns | 417.7 ns | 32.2 ns | 56 B |

两个结论都和直觉相反，所以写下来免得再被推翻一次：

- **`CreateDelegate` 的几百纳秒不是 JIT 预热**。测量时方法已经被绑成委托、已经 JIT 过了，
  仍然是 399 ns。把同一个方法换成程序集里的普通方法（不是 `DynamicMethod`）也一样是 403 ns
  ——代价来自 `Delegate.CreateDelegate` 本身的签名校验，跟载体无关。
- **多出来的那一跳反而更快**。包一层是两跳，但两跳都是普通调用；直接绑定只有一跳，可是把
  闭包塞进静态方法的第一个参数需要一段 shuffle thunk，每次调用要多挪一次参数。八次调用下来
  是 27.6 ns 对 32.2 ns。

代价是每次求值多一个显示类对象（88 B 而不是 56 B）。用 385 ns 换 32 B，不划算。

证据在 `ClosureBindingBenchmarks`。
...
| 谓词，有捕获（捕获一个 `int`） | — | 58.1 ns | 128 B |
| 块体谓词，有捕获（捕获 globals） | 13.5 ns | 59.5 ns | 128 B |
...
捕获型那 128 B 是闭包 40 B 加上绑定委托的 88 B（见 §6）。闭包这一半的构成取决于槽位表示：

| | 创建并写入一个槽 | 读 8 次 | 分配 |
|---|---:|---:|---:|
| `object[]` 装箱槽，捕获 `int` | 13.7 ns | 24.5 ns | 96 B |
| 强类型槽，捕获 `int` | **5.2 ns** | **21.8 ns** | **40 B** |
| `object[]` 装箱槽，捕获引用 | 11.6 ns | — | 72 B |
| 强类型槽，捕获引用 | **6.2 ns** | — | **40 B** |

引擎给整个脚本挑一种表示，所以端到端比较意味着两个进程，而两个隔了十几分钟的进程在这台机器上
分辨不了几个纳秒——上表两种表示是在**同一个进程里**手工发射并排测的（`ClosureSlotBenchmarks`），
读取那两行的误差是 ±0.17 ns。每次读省下约 0.34 ns（`ldfld` 对 `callvirt` + `ldelem.ref` +
`unbox.any`），真正的大头是创建：省掉一个数组对象，值类型再省一次装箱。
...
- 无格式说明符时脚本降级为 `string.Concat`，C# 编译器降级为 `DefaultInterpolatedStringHandler`。
  脚本反而更快，是因为这个规模下 handler 的缓冲区租借比直接 `Concat` 更贵，代价是多一次装箱
  （80 B vs 72 B）。
- 有格式说明符时两侧都走 handler，比值 1.02。这里原先降级为 `string.Format`，要在运行时解析
  复合格式串并把每个洞装箱，比值是 1.26；换成逐段 `AppendLiteral` / `AppendFormatted<T>` 之后
  差距基本消失。handler 是个 `ref struct` 局部，发射器本来就支持对值类型接收者取地址，所以这
  一项没有动发射器。对齐值解析不出整数、或运行时的 handler 形状对不上时，退回 `string.Format`。
...
这一节用的是默认任务而不是短任务：短任务只跑三次迭代，分辨不出这个量级上的差别。
...
要把 `decimal` 的整张静态方法表扫描并复制两遍。缓存键弱引用持有，缓存宿主类型不会钉住它所在的
程序集，宿主仍可以把规则装进可回收程序集再卸载。

| | 加缓存前 | 加缓存后 |
|---|---:|---:|
| 中等脚本耗时 | 32.7 µs | **27.6 µs** |
| 中等脚本分配 | 50.2 KB | **32.9 KB** |
| 小脚本分配 | 12.5 KB | **10.0 KB** |

小脚本的耗时没有变化（9.2 → 9.3 µs，在误差内）：它那 9 µs 由 `DynamicMethod` 的创建与委托构造
主导，绑定只占很小一块，省下的分配换不来时间。
