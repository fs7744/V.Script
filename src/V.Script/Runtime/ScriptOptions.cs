...
public sealed record ScriptOptions
{
    ...
    /// <summary>
    /// How to compile <c>await</c> and <c>async</c> lambdas, or null when only the base library
    /// is present. Installed by <c>V.Script.Async</c>'s <c>WithAsync()</c>.
    /// </summary>
    internal IAsyncSupport? AsyncSupport { get; init; }
